document.addEventListener('DOMContentLoaded', () => {
  // === DATA (boleh tambahkan userId agar match dgn reviews.js) ===
  const usersData = [
  { userId: 'u-101', name: 'Alex Johnson',    email: 'alex.j@example.com',     phone: '081234567890', source: 'Organic',  rating: 5, joinDate: '2025-08-15', orderHistory: [{ date: '2025-09-15', items: 3 }, { date: '2025-08-02', items: 1 }, { date: '2025-07-21', items: 2 }] },
  { userId: 'u-102', name: 'Samantha Bee',    email: 'samantha.b@example.com', phone: '081234567891', source: 'Referral', rating: 4, joinDate: '2025-09-01', orderHistory: [{ date: '2025-09-01', items: 5 }] },
  { userId: 'u-103', name: 'Charles Davis',   email: 'charles.d@example.com',  phone: '081234567892', source: 'Organic',  rating: 5, joinDate: '2025-01-20', orderHistory: [{ date: '2025-08-20', items: 2 }, { date: '2025-06-10', items: 4 }] },
];


  // === STATE ===
  // pinnedUserId: jika di-set, applyFilters akan membatasi hasil ke user ini saja
  ...
  const filterSelects = { source: document.getElementById('filter-source') };
  ...
  // === UTIL ===
  const uidFromEmail = (email) => {
    const h = Array.from(String(email)).reduce((acc, ch) => ((acc * 31) + ch.charCodeAt(0)) >>> 0, 0);
    return 'u-' + h.toString(36);
  };
  const getUid = (u) => u.userId || uidFromEmail(u.email);
  ...
  const renderTable = (users) => {
    ...
    tableBody.innerHTML = users.map(user => {
      ...
      return `
        <tr class="hover:bg-gray-50" data-user-id="${uid}">
          <td class="p-4 align-middle">
            <p class="font-medium text-gray-800">${user.name}</p>
            <p class="text-xs text-gray-500">${user.email}</p>
          </td>
          <td class="p-4 align-middle text-gray-600">
            <a href="whatsapp-leads.html?phone=${encodeURIComponent(user.phone)}" class="text-sky-600 hover:underline">
              ${user.phone}
            </a>
          </td>
  ...
  const renderFilterChips = (linkedUser) => {
    filterChipsContainer.innerHTML = ''; 
    let filterCount = 0;

    if (pinnedUserId && linkedUser) {
      filterCount++;
      const chip = document.createElement('div');
      chip.className = 'flex items-center gap-2 bg-sky-100 text-sky-800 text-sm font-medium px-3 py-1 rounded-full';
      chip.innerHTML = `<span>Linked from Review: ${linkedUser.name}</span><button class="remove-linked-btn text-sky-600 hover:text-sky-900" title="Clear">×</button>`;
      filterChipsContainer.appendChild(chip);

      chip.querySelector('.remove-linked-btn').addEventListener('click', () => {
        pinnedUserId = null;
        // kosongkan search agar kembali ke full list
        searchInput.value = '';
        applyFilters();
        // bersihkan query param ?userId (opsional)
  ...
  // === FILTER & SEARCH (respect pinnedUserId) ===
  const applyFilters = () => {
    const searchTerm = searchInput.value.toLowerCase();
    const sourceFilter = filterSelects.source.value;
    
    let filtered = usersData.filter(user => {
      // jika pinnedUserId ada, hanya tampilkan user tsb
      if (pinnedUserId && getUid(user) !== pinnedUserId) return false;
    ...
    // kalau pinned aktif, kirim info user buat chip
    renderFilterChips(pinnedUserId ? filtered[0] || usersData.find(u => getUid(u) === pinnedUserId) : null);
    updateKpiCards(filtered);

    // Tutup panel filter jika terbuka
    toggleFilterPanel(false);

    // Setelah render, jika pinned → auto-scroll & auto-open detail
    if (pinnedUserId) {
      const row = tableBody.querySelector(`[data-user-id="${pinnedUserId}"]`);
      if (row) {
        row.classList.add('ring-2','ring-sky-400','ring-offset-2','rounded');
        row.scrollIntoView({ behavior: 'smooth', block: 'center' });
        // Auto-open modal
        row.querySelector('.user-detail-btn')?.click();
        setTimeout(() => row.classList.remove('ring-2','ring-sky-400','ring-offset-2','rounded'), 4000);
      }
    }
  };
  ...
  const closeModal = (modal) => {
    ...
    if (modalContent) modalContent.classList.add('scale-95', '-translate-y-10');
  ...
  clearFiltersBtn.addEventListener('click', () => { filterSelects.source.value = 'all'; applyFilters(); });
  searchInput.addEventListener('input', applyFilters);
  
  document.addEventListener('click', (e) => { 
    if (!filterPanel.contains(e.target) && !filterMenuButton.contains(e.target)) toggleFilterPanel(false);
  });

  // Modal close handlers
  document.querySelectorAll('.modal').forEach(modal => {
    modal.addEventListener('click', (e) => {
      if (e.target.classList.contains('close-modal-btn') || e.target === modal) closeModal(modal);
    });
  });

  // Delegasi tombol tabel
  ...
  // === DEEP-LINK: filter ke user yg dimaksud ===
  const filterToUserFromUrl = () => {
    const params = new URLSearchParams(window.location.search);
    const uidParam = params.get('userId');
    if (!uidParam) return;

    // Cari user by uid langsung
    let target = usersData.find(u => getUid(u) === uidParam);

    // Fallback: kalau param ternyata email
    if (!target && uidParam.includes('@')) {
      target = usersData.find(u => u.email.toLowerCase() === uidParam.toLowerCase());
    }

    if (target) {
      pinnedUserId = getUid(target);              // aktifkan filter pin
      searchInput.value = target.email;           // isi search supaya terlihat jelas
      applyFilters();                              // render hanya 1 user + auto-open modal
    }
  };
  ...
  filterToUserFromUrl(); // panggil terakhir agar langsung terfilter ke user tsb jika ada
});
